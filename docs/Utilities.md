...
- **Actors (Arzachel, LOD, presets)**: each **`ActorElement`** can store **`ArzachelRigidBodyDamage`** (feeds the Arzachel **`Damaged`** pipeline at runtime), **LOD distance** hints, and **animation** / **destruction** preset **strings** alongside the glTF-backed **`MeshAsset`**. SMM’s **Arzachel, LOD, animation presets** section edits these; `EvaluateScene` fills **`ActorArzachelAuthorings`** for tools and the engine.
...
**Primitives + elements migration lane:** utility apps should call `LibUI::Widgets` for common controls (menus, popups, drag/scalar inputs, style scopes, key/mouse helpers) and prefer reusable `LibUI::Tools` composite elements like [`PropertyGrid.hxx`](../utilities/LibUI/Tools/PropertyGrid.hxx) for label/control form rows. Keep low-level `ImGui::*` calls inside LibUI internals or specialized draw-list code paths only.
