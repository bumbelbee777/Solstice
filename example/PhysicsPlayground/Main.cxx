...
int main() {
    try {
        ...
        // Create plane - static ground (both render and physics)
        ...
        // Create static ground RigidBody for ReactPhysics3D collision
        // The ground is a large box with top surface at y=0
        // Note: With ReactPhysics3D, we need an actual RigidBody component for collision
        // The box center is at y=-0.1 so the top surface (y=0.1) is at y=0
        auto groundEntityID = Registry.Create();
        auto& groundRB = Registry.Add<Physics::RigidBody>(groundEntityID);
        groundRB.Position = Vec3(0, -0.1f, 0); // Center at y=-0.1 so top is at y=0
        groundRB.Rotation = Quaternion(); // Identity rotation
        groundRB.IsStatic = true;
        groundRB.SetMass(0.0f); // Static body (infinite mass)
        groundRB.Type = Physics::ColliderType::Box;
        groundRB.HalfExtents = Vec3(500.0f, 0.1f, 500.0f); // Large flat box
        groundRB.Friction = 0.6f;
        groundRB.Restitution = 0.0f;
        groundRB.RenderObjectID = PlaneObjID; // Link to render object

        // Force sync to create the ground body in ReactPhysics3D immediately
        // This ensures it exists before dynamic bodies are created
        ...
        // Tetrahedron
        {
            ...
            auto tetraMesh = Arzachel::MeshFactory::CreateTetrahedron(0.5f);
            ...
            rb.SetMass(0.8f);
            float radius = std::sqrt(0.5f * 0.5f + 0.5f * 0.5f + 0.5f * 0.5f); // ≈ 0.866f
            ...
            // Use Tetrahedron collider type
            rb.Type = Physics::ColliderType::Tetrahedron;
            // Set up tetrahedron vertices from the convex hull
            if (convexHull && !convexHull->Vertices.empty()) {
                rb.HullVertices = convexHull->Vertices; // Copy vertices for tetrahedron support point calculation
            }
            rb.Hull = convexHull; // Also keep the hull for AABB computation
        ...
        // Cylinder
        {
            ...
            // Ensure cylinder starts with identity rotation (upright)
            rb.Rotation = Quaternion(); // Identity quaternion (w=1, x=0, y=0, z=0)

            // Cylinder inertia: For a cylinder with radius R and height H, mass m:
            // Ixx = Izz = m/12 * (3*R^2 + H^2) (rotation about x or z axis)
            // Iyy = m * R^2 / 2 (rotation about y axis, the cylinder's main axis)
            float mass = 1.5f;
            float radius = 0.5f;
            float height = 1.0f;
            rb.SetMass(mass);
            ...
            float Izz = Ixx; // Same as Ixx for symmetry
            rb.InverseInertiaTensor = Vec3(1.0f / Ixx, 1.0f / Iyy, 1.0f / Izz);

            rb.Restitution = 0.3f;
            rb.Friction = 0.6f; // Increased friction for better rolling resistance
            rb.AngularDrag = 0.4f; // Increased angular drag to prevent tilting and perpetual rolling
            rb.LinearDamping = 0.1f; // Increased linear damping to help settle faster
            rb.Type = Physics::ColliderType::Cylinder;
            rb.CylinderRadius = radius;
            rb.CylinderHeight = height;
            rb.Radius = std::sqrt(radius * radius + (height * 0.5f) * (height * 0.5f)); // Bounding radius for AABB
            rb.RenderObjectID = renderID;
            dynamicObjects.push_back({renderID, entityID});
        }
