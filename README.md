...
A CPU-first game engine for projects that want PS2-era constraints with modern throughput: low internal resolutions, baked or lightly dynamic lighting, and software rasterization, but with SIMD, SoA layouts, and job-style parallelism where it pays off. Optional GPU paths exist (bgfx), but the design assumes most if not all the interesting work happens on the CPU.
...
The tree is quite large. In practice Solstice bundles:
...
- **Everything else:** ECS, Moonwalk scripting VM, ImGui UI and motion graphics helpers, Arzachel procedural meshes, navmesh pathfinding, small NN utilities, custom spatial audio via SDL_mixer, saves and profiling (including flame graph export).

If you need API-level detail, start with the docs under `docs/` rather than this file.

## Design stance

Solstice is not trying to be a general-purpose “modern PBR slop engine.” It is deliberately scoped: cap resolution and texture size, spend budget on culling and layout, and use the CPU hard. The sweet spot is stylized 3D or “next-gen retro” games at roughly **720p** on a decent multicore machine, with **1080p** as more of a stress or demo mode.
...
| [docs/Packaging.md](docs/Packaging.md) | FST manifests and zip/deb/msi for utilities and games |

## Utilities

- **Jackhammer (LevelEditor)** — Hammer-style BSP and octree-based level editor
- **SMM (MovieMaker)** — SFM-inspired 2D and 3D animation tool
- **Sharpon** — Script editor for Moonwalk
...
Dependencies are pulled at configure time with **CPM**; you don't need a manual 3rdparty checkout for a normal build.
...
bgfx (bx, bimg), SDL3, SDL_mixer, ReactPhysics3D, ImGui, tinygltf (and nlohmann/json through it), ffmpeg/libav (optional), plus other transitive fetches as the CMake graph requires.
