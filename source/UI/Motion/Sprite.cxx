...
void Sprite::Render(ImDrawList* DrawList) {
    ...
    const ImTextureID texId = static_cast<ImTextureID>(static_cast<uintptr_t>(m_Texture.idx));

    if (std::fabs(m_Rotation) < 1e-5f) {
        ImVec2 min = m_Position;
        ImVec2 max = ImVec2(m_Position.x + m_Size.x, m_Position.y + m_Size.y);
        DrawList->AddImage(texId, min, max, m_Frame.UV0, m_Frame.UV1, color);
        return;
    }

    const float cx = m_Position.x + m_Size.x * 0.5f;
    const float cy = m_Position.y + m_Size.y * 0.5f;
    ...
    const ImVec2 p1 = rot(-hw, -hh);
    const ImVec2 p2 = rot(hw, -hh);
    const ImVec2 p3 = rot(hw, hh);
    const ImVec2 p4 = rot(-hw, hh);
    const ImVec2 uv0 = m_Frame.UV0;
    const ImVec2 uv1 = ImVec2(m_Frame.UV1.x, m_Frame.UV0.y);
    const ImVec2 uv2 = m_Frame.UV1;
    const ImVec2 uv3 = ImVec2(m_Frame.UV0.x, m_Frame.UV1.y);
    DrawList->AddImageQuad(texId, p1, p2, p3, p4, uv0, uv1, uv2, uv3, color);
}
