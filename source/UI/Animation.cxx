...
float Animation::Ease(float T, EasingType Type, float Strength) {
    // Clamp T to [0, 1]
    ...
    switch (Type) {
        ...
        case EasingType::EaseOut:
            result = EaseOutQuad(T);
            break;
        case EasingType::EaseInOut:
            result = EaseInOutQuad(T);
            break;
        ...
        case EasingType::Quart:
            result = EaseInOutQuart(T);
            break;
        case EasingType::Quint:
            result = EaseInOutQuint(T);
            break;
        case EasingType::Expo:
            result = EaseInOutExpo(T);
            break;
        case EasingType::Circ:
            result = EaseInOutCirc(T);
            break;
        case EasingType::Back:
            result = EaseInOutBack(T);
            break;
        case EasingType::Elastic:
            result = EaseInOutElastic(T);
            break;
        case EasingType::Bounce:
            result = EaseInOutBounce(T);
            break;
        case EasingType::Bezier:
            // Default bezier curve (ease-in-out cubic)
            result = EaseBezier(T, 0.42f, 0.0f, 0.58f, 1.0f);
            break;
    }

    // Apply strength (for elastic/bounce effects)
...
float Animation::EaseBezier(float T, float P1X, float P1Y, float P2X, float P2Y) {
    // Clamp T to [0, 1]
    T = std::max(0.0f, std::min(1.0f, T));
    ...
    // We need to solve for t given x, then evaluate y
    // For UI purposes, we'll use a simplified approach: binary search
    // This is not the most efficient but works well for UI animations
    ...
    for (int i = 0; i < 10; ++i) {
        mid = (low + high) * 0.5f;
        float x = uuu * 0.0f + 3.0f * uu * mid * P1X + 3.0f * u * mid * mid * P2X + mid * mid * mid * 1.0f;
    ...
    float y = uuu * 0.0f + 3.0f * uu * mid * P1Y + 3.0f * u * mid * mid * P2Y + mid * mid * mid * 1.0f;

    return y;
}

// ShadowParams Lerp implementation - non-template overload
// This is called when T = ShadowParams via overload resolution
...
} // namespace Solstice::UI::Animation
