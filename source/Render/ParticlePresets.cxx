...
void SnowParticleSystemImpl::UpdateParticle(Particle& Particle, float Dt) {
    ...
    if (Particle.Life < 2.0f) {
        Particle.Alpha = Particle.Life / 2.0f;
    }
}

void SnowParticleSystemImpl::BuildVertexData(const Particle& Particle, std::vector<uint8_t>& Vertices, size_t& VertexOffset) {
    ...
    float OffsetX = static_cast<float>((PosHash & 0xFF) - 128) / 128.0f * 0.2f; // Increased from 0.1f
    float OffsetZ = static_cast<float>(((PosHash >> 8) & 0xFF) - 128) / 128.0f * 0.2f;
    float OffsetY = static_cast<float>(((PosHash >> 16) & 0xFF) - 128) / 128.0f * 0.1f; // Add Y offset too

    // Create 4 vertices for a quad (billboard) with improved offsets to reduce line artifacts
    // The billboard should be camera-facing (handled by shader), but we add position offsets
    // to break up any alignment patterns
    ParticleVertex Quad[4] = {
        {Particle.Position.x + OffsetX, Particle.Position.y + OffsetY, Particle.Position.z + OffsetZ, R, G, B, Alpha, 0.0f, 1.0f}, // Bottom-left
        {Particle.Position.x + Size + OffsetX, Particle.Position.y + OffsetY, Particle.Position.z + OffsetZ, R, G, B, Alpha, 1.0f, 1.0f}, // Bottom-right
        {Particle.Position.x + OffsetX, Particle.Position.y + Size + OffsetY, Particle.Position.z + OffsetZ, R, G, B, Alpha, 0.0f, 0.0f}, // Top-left
        {Particle.Position.x + Size + OffsetX, Particle.Position.y + Size + OffsetY, Particle.Position.z + OffsetZ, R, G, B, Alpha, 1.0f, 0.0f} // Top-right
    };
    ...
    for (int I = 0; I < 4; ++I) {
        if (VertexOffset + VertexSize <= Vertices.size()) {
            std::memcpy(Vertices.data() + VertexOffset, &Quad[I], VertexSize);
...
void UnifiedParticleSystemImpl::SpawnParticle(const Math::Vec3& SpawnPos) {
    ...
    Particle.Alpha = (m_Type == ParticlePresetType::Electricity) ? elecData->Alpha : 1.0f;
...
void UnifiedParticleSystemImpl::UpdateParticle(Particle& Particle, float Dt) {
    switch (m_Type) {
        ...
        case ParticlePresetType::Fire: {
            ...
            // Size growth
            Particle.Size += Dt * fireData->SizeGrowthRate * 0.1f;
            break;
        }
        case ParticlePresetType::Electricity: {
            const ElectricityData* elecData = GetPresetData<ElectricityData>();
        ...
        case ParticlePresetType::Smoke: {
            ...
            Particle.Size += Dt * smokeData->SizeGrowthRate * 0.1f;
            break;
        }
        case ParticlePresetType::ToxicGas: {
            const ToxicGasData* gasData = GetPresetData<ToxicGasData>();
            Particle.Velocity += m_WindDirection * m_WindStrength * Dt * 0.2f;
            Particle.Size += Dt * gasData->ExpansionRate * 0.1f;
            break;
        }
    }
    ...
    switch (m_Type) {
        ...
        case ParticlePresetType::Smoke: {
            const SmokeData* smokeData = GetPresetData<SmokeData>();
            Particle.Alpha = lifeRatio * smokeData->DensityOpacity;
            break;
        }
        case ParticlePresetType::ToxicGas: {
            const ToxicGasData* gasData = GetPresetData<ToxicGasData>();
            Particle.Alpha = lifeRatio * gasData->DensityVisibility;
            break;
        }
        default: {
            if (Particle.Life < 2.0f) {
...
void UnifiedParticleSystemImpl::BuildVertexData(const Particle& Particle, std::vector<uint8_t>& Vertices, size_t& VertexOffset) {
    struct ParticleVertex {
        float X, Y, Z;
        uint8_t R, G, B, A;
        float U, V;
    ...
    ParticleVertex Quad[4] = {
        {Particle.Position.x, Particle.Position.y, Particle.Position.z, R, G, B, Alpha, 0.0f, 1.0f},
        {Particle.Position.x + Size, Particle.Position.y, Particle.Position.z, R, G, B, Alpha, 1.0f, 1.0f},
        {Particle.Position.x, Particle.Position.y + Size, Particle.Position.z, R, G, B, Alpha, 0.0f, 0.0f},
        {Particle.Position.x + Size, Particle.Position.y + Size, Particle.Position.z, R, G, B, Alpha, 1.0f, 0.0f}
    };
