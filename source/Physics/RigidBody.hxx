...
struct AABB {
    ...
    // Helper to check intersection (SIMD optimized)
    bool Intersects(const AABB& other) const {
        using Solstice::Core::SIMD::Vec4;
        Vec4 thisMin(Min.x, Min.y, Min.z, 0.0f);
        Vec4 thisMax(Max.x, Max.y, Max.z, 0.0f);
        Vec4 otherMin(other.Min.x, other.Min.y, other.Min.z, 0.0f);
        Vec4 otherMax(other.Max.x, other.Max.y, other.Max.z, 0.0f);

        // Check: thisMin <= otherMax && thisMax >= otherMin
        // For SIMD, we can check all components at once
        // Note: Vec4 doesn't have comparison operators, so we'll use scalar for now
        // but structure allows for future SIMD comparison optimization
...
struct RigidBody {
    ...
    // Cylinder
    float CylinderHeight{1.0f};     // Height of cylinder
    ...
    // Sleep mechanism
    bool IsAsleep{false};            // Whether body is sleeping
    int SleepCounter{0};             // Frames below threshold
