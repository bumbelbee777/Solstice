...
#include "ConvexHull.hxx"  // Must be included before using ConvexHull
#include "ShapeHelpers.hxx"  // For TransformPoint helper
...
void ReactPhysics3DBridge::SyncToReactPhysics3D() {
    ...
    // Collect entities to remove first to avoid iterator invalidation
    std::vector<ECS::EntityId> entitiesToRemove;
    entitiesToRemove.reserve(8); // Pre-allocate small buffer

    // Single pass: check for new bodies and sync existing ones
    // First, check for new RigidBody components that need bodies created
    m_Registry->ForEach<RigidBody>([this, &entitiesToRemove](ECS::EntityId entityId, RigidBody& rigidBody) {
        auto it = m_EntityToBody.find(entityId);
        if (it == m_EntityToBody.end()) {
            // New body, create it
            CreateBody(entityId, rigidBody);
        } else {
            // Existing body, sync properties
            reactphysics3d::RigidBody* rp3dBody = it->second;
            if (!m_Registry->Valid(entityId) || !m_Registry->Has<RigidBody>(entityId)) {
                // Entity was removed, mark for cleanup
                entitiesToRemove.push_back(entityId);
            } else {
                UpdateBodyProperties(rp3dBody, rigidBody);
            }
        }
    });
...
void ReactPhysics3DBridge::RemoveBody(ECS::EntityId entityId) {
    ...
    // Clean up shape if we created it
    auto shapeIt = m_BodyToShape.find(rp3dBody);
    if (shapeIt != m_BodyToShape.end()) {
        reactphysics3d::CollisionShape* shape = shapeIt->second;

        // Delete shape based on type - order by frequency (most common first)
        // Note: dynamic_cast is necessary here as ReactPhysics3D doesn't expose a type enum
