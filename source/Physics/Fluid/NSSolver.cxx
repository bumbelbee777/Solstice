#include <Physics/Fluid/NSSolver.hxx>
#include <Physics/Fluid/FluidSparse.hxx>
#include <Physics/Fluid/FluidFFT.hxx>
...
public:
    explicit ScopedProfile(const char* name) : name_(name) {
        Profiler::Instance().BeginScope(name_);
    }
    ~ScopedProfile() {
        Profiler::Instance().EndScope(name_);
    }
...
#define NSS_PROFILE_SCOPE(nameLiteral) ScopedProfile scopedProfile_##__LINE__(nameLiteral)
...
void SmoothPressurePoisson(std::vector<float>& p, const std::vector<float>& rhs, const GridLayout& g, int iterations,
                           bool parallelRedBlack) {
    const int sx = g.sx();
    const int sxy = g.sxy();
    ...
    if (!parallelRedBlack) {
        for (int it = 0; it < iterations; ++it) {
            ...
            SetBoundariesPtrImpl(0, p.data(), g);
...
#pragma omp parallel
            {
                float threadMaxDelta = 0.0f;
#pragma omp for schedule(static)
                for (int k = 1; k <= g.Nz; ++k) {
                    for (int j = 1; j <= g.Ny; ++j) {
                        const int rowStart = g.IX(1, j, k);
                        const int iStart = 1 + (((j + k + color) & 1) != 0 ? 1 : 0);
                        for (int i = iStart; i <= g.Nx; i += 2) {
                            ...
                            newValue = SanitizeFinite(newValue);
                            p[static_cast<size_t>(idx)] = newValue;
                            threadMaxDelta = std::max(threadMaxDelta, std::abs(newValue - oldValue));
                        }
                    }
                }
#pragma omp critical
                { maxDelta = std::max(maxDelta, threadMaxDelta); }
            }
#else
            for (int k = 1; k <= g.Nz; ++k) {
                for (int j = 1; j <= g.Ny; ++j) {
...
void ComputePressureResidual(std::vector<float>& residual, const std::vector<float>& p, const std::vector<float>& rhs,
                             const GridLayout& g) {
    const int sx = g.sx();
    const int sxy = g.sxy();
    const float wx = 1.0f / (g.hx * g.hx);
    const float wy = 1.0f / (g.hy * g.hy);
    const float wz = 1.0f / (g.hz * g.hz);
    const float diag = 2.0f * (wx + wy + wz);
    std::fill(residual.begin(), residual.end(), 0.0f);
    for (int k = 1; k <= g.Nz; ++k) {
        for (int j = 1; j <= g.Ny; ++j) {
            for (int i = 1; i <= g.Nx; ++i) {
                const int idx = g.IX(i, j, k);
                const float lap = diag * p[static_cast<size_t>(idx)]
                    - wx * (p[static_cast<size_t>(idx + 1)] + p[static_cast<size_t>(idx - 1)])
                    - wy * (p[static_cast<size_t>(idx + sx)] + p[static_cast<size_t>(idx - sx)])
                    - wz * (p[static_cast<size_t>(idx + sxy)] + p[static_cast<size_t>(idx - sxy)]);
                residual[static_cast<size_t>(idx)] = SanitizeFinite(rhs[static_cast<size_t>(idx)] - lap);
            }
        }
    }
    SetBoundariesPtrImpl(0, residual.data(), g);
}

void RestrictResidual(const std::vector<float>& fineResidual, const GridLayout& fine, std::vector<float>& coarseRhs,
                      const GridLayout& coarse) {
    std::fill(coarseRhs.begin(), coarseRhs.end(), 0.0f);
    for (int kc = 1; kc <= coarse.Nz; ++kc) {
        const int kfBase = 2 * kc - 1;
        for (int jc = 1; jc <= coarse.Ny; ++jc) {
            const int jfBase = 2 * jc - 1;
            for (int ic = 1; ic <= coarse.Nx; ++ic) {
                const int ifBase = 2 * ic - 1;
                float sum = 0.0f;
                int count = 0;
                for (int dz = 0; dz <= 1; ++dz) {
                    const int kf = std::min(kfBase + dz, fine.Nz);
                    for (int dy = 0; dy <= 1; ++dy) {
                        const int jf = std::min(jfBase + dy, fine.Ny);
                        for (int dx = 0; dx <= 1; ++dx) {
                            const int ifv = std::min(ifBase + dx, fine.Nx);
                            sum += fineResidual[static_cast<size_t>(fine.IX(ifv, jf, kf))];
                            ++count;
                        }
                    }
                }
                coarseRhs[static_cast<size_t>(coarse.IX(ic, jc, kc))] = SanitizeFinite(sum / static_cast<float>(count));
            }
        }
    }
    SetBoundariesPtrImpl(0, coarseRhs.data(), coarse);
}

void ProlongateAdd(const std::vector<float>& coarsePressure, const GridLayout& coarse, std::vector<float>& finePressure,
                   const GridLayout& fine) {
    for (int kf = 1; kf <= fine.Nz; ++kf) {
        const int kc = std::min(coarse.Nz, (kf + 1) / 2);
        for (int jf = 1; jf <= fine.Ny; ++jf) {
            const int jc = std::min(coarse.Ny, (jf + 1) / 2);
            for (int ifv = 1; ifv <= fine.Nx; ++ifv) {
                const int ic = std::min(coarse.Nx, (ifv + 1) / 2);
                const int fIdx = fine.IX(ifv, jf, kf);
                const int cIdx = coarse.IX(ic, jc, kc);
                finePressure[static_cast<size_t>(fIdx)] += coarsePressure[static_cast<size_t>(cIdx)];
            }
        }
    }
    SetBoundariesPtrImpl(0, finePressure.data(), fine);
}

std::vector<PressureMultigridLevel> BuildPressureMultigridLevels(const GridLayout& finest, int requestedLevels) {
    std::vector<PressureMultigridLevel> levels;
    levels.reserve(static_cast<size_t>(std::max(requestedLevels, 1)));

    GridLayout current = finest;
    for (int level = 0; level < std::max(requestedLevels, 1); ++level) {
        PressureMultigridLevel mgLevel;
        mgLevel.Grid = current;
        const size_t cellCount = current.CellCountPadded();
        mgLevel.Pressure.assign(cellCount, 0.0f);
        mgLevel.Rhs.assign(cellCount, 0.0f);
        mgLevel.Residual.assign(cellCount, 0.0f);
        levels.push_back(std::move(mgLevel));
        if (current.Nx <= 2 || current.Ny <= 2 || current.Nz <= 2) {
            break;
        }
        current = GridLayout::FromBox(
            std::max(1, current.Nx / 2), std::max(1, current.Ny / 2), std::max(1, current.Nz / 2), current.hx * 2.0f,
            current.hy * 2.0f, current.hz * 2.0f
        );
    }
    return levels;
}

void RunPressureMultigridVcycle(std::vector<PressureMultigridLevel>& levels, size_t levelIndex, int preSmooth,
                                int postSmooth, int coarseSmooth, bool parallelRedBlack) {
    auto& level = levels[levelIndex];
    if (levelIndex + 1 >= levels.size()) {
        SmoothPressurePoisson(level.Pressure, level.Rhs, level.Grid, coarseSmooth, parallelRedBlack);
        return;
    }

    SmoothPressurePoisson(level.Pressure, level.Rhs, level.Grid, preSmooth, parallelRedBlack);
    ComputePressureResidual(level.Residual, level.Pressure, level.Rhs, level.Grid);

    auto& coarse = levels[levelIndex + 1];
    RestrictResidual(level.Residual, level.Grid, coarse.Rhs, coarse.Grid);
    std::fill(coarse.Pressure.begin(), coarse.Pressure.end(), 0.0f);
    SetBoundariesPtrImpl(0, coarse.Pressure.data(), coarse.Grid);

    RunPressureMultigridVcycle(levels, levelIndex + 1, preSmooth, postSmooth, coarseSmooth, parallelRedBlack);
    ProlongateAdd(coarse.Pressure, coarse.Grid, level.Pressure, level.Grid);
    SmoothPressurePoisson(level.Pressure, level.Rhs, level.Grid, postSmooth, parallelRedBlack);
}
...
void NSSolver::LinearSolveAnisotropicDiffuse(int b, std::vector<float>& x, const std::vector<float>& x0, float ax, float ay,
                                           float az, const GridLayout& g, bool parallelRedBlack) {
    const int sx = g.sx();
    const int sxy = g.sxy();
    const float denom = 1.0f + 2.0f * (ax + ay + az);
    const float invD = 1.0f / denom;
...
void NSSolver::LinearSolveAnisotropicPressure(std::vector<float>& p, const std::vector<float>& div, const GridLayout& g,
                                              const FluidSolverTuning& tuning, bool parallelRedBlack) {
    const int relaxIterations = ClampIntRange(
        tuning.pressureRelaxationIterations > 0 ? tuning.pressureRelaxationIterations : kMaxLinearSolveIterations, 4, 512
    );
    const int requestedLevels = ClampIntRange(tuning.pressureMultigridLevels, 2, 8);
    const int preSmooth = ClampIntRange(tuning.pressureMultigridPreSmooth, 1, 10);
    const int postSmooth = ClampIntRange(tuning.pressureMultigridPostSmooth, 1, 10);
    const int coarseSmooth = ClampIntRange(tuning.pressureMultigridCoarseSmooth, 4, 256);

    const bool useMultigrid = tuning.usePressureMultigrid && requestedLevels > 1 && g.Nx >= 4 && g.Ny >= 4 && g.Nz >= 4;
    if (!useMultigrid) {
        SmoothPressurePoisson(p, div, g, relaxIterations, parallelRedBlack);
        return;
    }

    auto levels = BuildPressureMultigridLevels(g, requestedLevels);
    if (levels.size() <= 1) {
        SmoothPressurePoisson(p, div, g, relaxIterations, parallelRedBlack);
        return;
    }

    levels.front().Pressure = p;
    levels.front().Rhs = div;
    SetBoundariesPtrImpl(0, levels.front().Pressure.data(), g);
    SetBoundariesPtrImpl(0, levels.front().Rhs.data(), g);

    const int vCycleCount = std::max(2, relaxIterations / std::max(1, preSmooth + postSmooth));
    for (int cycle = 0; cycle < vCycleCount; ++cycle) {
        RunPressureMultigridVcycle(levels, 0, preSmooth, postSmooth, coarseSmooth, parallelRedBlack);
    }
    SmoothPressurePoisson(levels.front().Pressure, levels.front().Rhs, g, std::max(2, preSmooth), parallelRedBlack);
    p.swap(levels.front().Pressure);
}

void NSSolver::LinearSolveIsotropic(int b, std::vector<float>& x, const std::vector<float>& x0, float a, float c,
                                    const GridLayout& g, int iterations) {
    const int N = g.Nx;
    const int s = g.sx();
    const int s2 = g.sxy();
    const float invC = 1.0f / c;
    const int iterationCount = ClampIntRange(iterations > 0 ? iterations : kMaxLinearSolveIterations, 2, 512);
...
void NSSolver::DiffuseSpectralPeriodic(int b, std::vector<float>& x, const std::vector<float>& x0, float nu, float dt,
                                       const GridLayout& g, std::vector<std::complex<float>>* spectralScratch) {
    if (!g.LegacyIsotropicCube()) {
    ...
    const int n3 = N * N * N;
    std::vector<std::complex<float>> localWork;
    std::vector<std::complex<float>>* workRef = spectralScratch;
    if (!workRef) {
        localWork.resize(static_cast<size_t>(n3));
        workRef = &localWork;
    } else if (workRef->size() != static_cast<size_t>(n3)) {
        workRef->resize(static_cast<size_t>(n3));
    }
    auto& work = *workRef;
#ifdef SOLSTICE_HAS_OPENMP
#pragma omp parallel for schedule(static)
...
void NSSolver::Diffuse(int b, std::vector<float>& x, const std::vector<float>& x0, float diff, float dt, const GridLayout& g,
    ...
    if (g.LegacyIsotropicCube()) {
        ...
        LinearSolveAnisotropicDiffuse(b, x, x0, ax, ay, az, g, parallelAnisotropic);
    }
}

void NSSolver::Project(std::vector<float>& velocX, std::vector<float>& velocY, std::vector<float>& velocZ, std::vector<float>& p,
                       std::vector<float>& div, const GridLayout& g, const FluidSolverTuning& tuning,
                       bool parallelAnisotropic) {
    if (!g.LegacyIsotropicCube()) {
...
#pragma omp parallel for schedule(static)
        ...
        for (int k = 1; k <= g.Nz; ++k) {
            for (int j = 1; j <= g.Ny; ++j) {
                ...
                for (; i <= g.Nx; ++i) {
                    const int idx = g.IX(i, j, k);
                    const int idxXp = idx + 1;
                    const int idxXm = idx - 1;
                    const int idxYp = idx + sx;
                    const int idxYm = idx - sx;
                    const int idxZp = idx + sxy;
                    const int idxZm = idx - sxy;
                    div[static_cast<size_t>(idx)] =
                        -((velocX[static_cast<size_t>(idxXp)] - velocX[static_cast<size_t>(idxXm)]) * i2hx
                          + (velocY[static_cast<size_t>(idxYp)] - velocY[static_cast<size_t>(idxYm)]) * i2hy
                          + (velocZ[static_cast<size_t>(idxZp)] - velocZ[static_cast<size_t>(idxZm)]) * i2hz);
        ...
        SetBoundaries(0, p, g);
        LinearSolveAnisotropicPressure(p, div, g, tuning, parallelAnisotropic);
#ifdef SOLSTICE_HAS_OPENMP
#pragma omp parallel for schedule(static)
#endif
        for (int k = 1; k <= g.Nz; ++k) {
            for (int j = 1; j <= g.Ny; ++j) {
                ...
                for (; i <= g.Nx; ++i) {
                    ...
                    const int idxZp = idx + sxy;
                    const int idxZm = idx - sxy;
                    velocX[static_cast<size_t>(idx)] -= (p[static_cast<size_t>(idxXp)] - p[static_cast<size_t>(idxXm)]) * i2hx;
                    velocY[static_cast<size_t>(idx)] -= (p[static_cast<size_t>(idxYp)] - p[static_cast<size_t>(idxYm)]) * i2hy;
                    velocZ[static_cast<size_t>(idx)] -= (p[static_cast<size_t>(idxZp)] - p[static_cast<size_t>(idxZm)]) * i2hz;
...
#pragma omp parallel for schedule(static)
    ...
    const int isotropicPressureIters = std::max(
        64, tuning.pressureRelaxationIterations > 0 ? tuning.pressureRelaxationIterations : kMaxLinearSolveIterations
    );
    LinearSolveIsotropic(0, p, div, 1.0f, 6.0f, g, isotropicPressureIters);
...
void NSSolver::ApplyTemporalReprojection(std::vector<float>& density, const std::vector<float>& history, const std::vector<float>& u,
                                         const std::vector<float>& v, const std::vector<float>& w, float dt, const GridLayout& g,
                                         float alpha, float* reprojectionScratch, size_t reprojectionScratchCount) {
    ...
    const float lz = 0.5f;
    const float hzb = static_cast<float>(g.Nz) + 0.5f;
    if (!reprojectionScratch || reprojectionScratchCount < density.size()) {
        return;
    }
    float* tmp = reprojectionScratch;
    const Vec4 vdtx = Vec4Splat(dtx);
    const Vec4 vdty = Vec4Splat(dty);
    const Vec4 vdtz = Vec4Splat(dtz);
#ifdef SOLSTICE_HAS_OPENMP
    ...
    for (int k = 1; k <= g.Nz; k++) {
        for (int j = 1; j <= g.Ny; j++) {
            ...
            for (; i <= g.Nx - 3; i += 4) {
                ...
                ClampCellCoordArray4(pz, lz, hzb);
                for (int q = 0; q < 4; ++q) {
                    tmp[idx + q] = SampleTrilinearField(history, px[q], py[q], pz[q], g);
                }
            }
            for (; i <= g.Nx; ++i) {
                const int idx = g.IX(i, j, k);
        ...
        for (int j = 1; j <= g.Ny; j++) {
            for (int i = 1; i <= g.Nx; i++) {
                ...
                density[static_cast<size_t>(idx)] =
                    SanitizeFinite((1.0f - wgt) * density[static_cast<size_t>(idx)] + wgt * tmp[idx]);
            }
        }
    }
}
...
void NSSolver::Resolve(FluidSimulation& fluid, float dt) {
    NSS_PROFILE_SCOPE("NSSolver.Resolve");
    const GridLayout grid = fluid.GetGridLayout();
    if (grid.Nx <= 0 || grid.Ny <= 0 || grid.Nz <= 0 || !std::isfinite(dt) || dt <= 0.0f) {
        return;
    }
    Profiler::Instance().SetCounter("NSSolver.Cells", static_cast<int64_t>(grid.Nx) * static_cast<int64_t>(grid.Ny) * static_cast<int64_t>(grid.Nz));
    ...
    const bool parallelAnisotropic = tuning.parallelAnisotropicLinearSolve;
    std::vector<std::complex<float>>* spectralScratch = &fluid.spectralScratch;
    float* reprojectionScratch = fluid.temporalReprojectionScratch.Data();
    const size_t reprojectionScratchCount = fluid.temporalReprojectionScratch.Size();

    FluidSparseData* sparsePtr = fluid.sparse.get();
    if (sparsePtr && tuning.useSparseRaymarch) {
        NSS_PROFILE_SCOPE("NSSolver.SparseBuild");
    ...
    {
        NSS_PROFILE_SCOPE("NSSolver.VelocityDiffuse");
        if (spectralVel) {
            DiffuseSpectralPeriodic(1, Vx0, Vx, visc, dt, grid, spectralScratch);
            DiffuseSpectralPeriodic(2, Vy0, Vy, visc, dt, grid, spectralScratch);
            DiffuseSpectralPeriodic(3, Vz0, Vz, visc, dt, grid, spectralScratch);
        } else {
            Diffuse(1, Vx0, Vx, visc, dt, grid, parallelAnisotropic);
            Diffuse(2, Vy0, Vy, visc, dt, grid, parallelAnisotropic);
            Diffuse(3, Vz0, Vz, visc, dt, grid, parallelAnisotropic);
        }
    }

    {
        NSS_PROFILE_SCOPE("NSSolver.ProjectVelocity0");
        Project(Vx0, Vy0, Vz0, pressure, divergence, grid, tuning, parallelAnisotropic);
    }
    {
        NSS_PROFILE_SCOPE("NSSolver.Boussinesq");
        ApplyBoussinesq(fluid, dt, grid);
    }

    const bool raymarch = tuning.useSparseRaymarch;
    const FluidSparseData* sparseConst = sparsePtr;
    Profiler::Instance().SetCounter("NSSolver.UseMacCormack", tuning.enableMacCormack ? 1 : 0);
    Profiler::Instance().SetCounter("NSSolver.UseSparseRaymarch", raymarch ? 1 : 0);
    Profiler::Instance().SetCounter("NSSolver.UseSpectral", spectralOk ? 1 : 0);
    Profiler::Instance().SetCounter("NSSolver.UseParallelAnisotropicSolve", parallelAnisotropic ? 1 : 0);
    Profiler::Instance().SetCounter("NSSolver.UsePressureMultigrid", tuning.usePressureMultigrid ? 1 : 0);
    Profiler::Instance().SetCounter("NSSolver.PressureMGLevels", tuning.pressureMultigridLevels);

    {
        NSS_PROFILE_SCOPE("NSSolver.VelocityAdvect");
        if (raymarch) {
    ...
    {
        NSS_PROFILE_SCOPE("NSSolver.ProjectVelocity1");
        Project(Vx, Vy, Vz, pressure, divergence, grid, tuning, parallelAnisotropic);
    }

    if (thermal.enableBoussinesq) {
        NSS_PROFILE_SCOPE("NSSolver.Thermal");
    ...
    {
        NSS_PROFILE_SCOPE("NSSolver.DensityDiffuse");
        if (spectralDen) {
            DiffuseSpectralPeriodic(0, density_prev, dens, diff, dt, grid, spectralScratch);
        } else {
            Diffuse(0, density_prev, dens, diff, dt, grid, parallelAnisotropic);
        }
    }

    {
        NSS_PROFILE_SCOPE("NSSolver.DensityAdvect");
        if (raymarch) {
    ...
    if (fluid.densityHistoryValid && tuning.temporalReprojectWeight > 0.0f) {
        NSS_PROFILE_SCOPE("NSSolver.TemporalReproject");
        ApplyTemporalReprojection(
            dens, density_history, Vx, Vy, Vz, dt, grid, tuning.temporalReprojectWeight, reprojectionScratch,
            reprojectionScratchCount
        );
    }

    {
        NSS_PROFILE_SCOPE("NSSolver.Bitplane");
