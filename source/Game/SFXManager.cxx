    ...
    if (m_AudioManager) {
        for (auto& sound : m_ActiveSounds) {
            if (sound.Source.Track) {
                m_AudioManager->UpdateAudioSource(sound.Source);
            }
        }
    }
    ...
    m_ActiveSounds.erase(
        std::remove_if(m_ActiveSounds.begin(), m_ActiveSounds.end(),
            [](const SoundEffect& sound) {
                return sound.Source.Track == nullptr && !sound.IsLooping;
            }),
        m_ActiveSounds.end()
    );
}
...
Core::Audio::AudioSource SFXManager::PlaySound3D(const std::string& Path, const Math::Vec3& Position,
    ...
    float effectiveVolume = GetEffectiveVolume(Category, Volume);
    Core::Audio::AudioSource source = m_AudioManager->PlaySound3D(Path.c_str(), Position, MaxDistance, Loop);
...
void SFXManager::PlaySoundFromPool(const std::string& Path, SFXCategory Category) {
    ...
    if (pool && m_AudioManager) {
        ...
        Core::Audio::AudioSource& source = pool->Sources[pool->CurrentIndex];
        ...
        // Play new sound
        source = m_AudioManager->PlaySound3D(Path.c_str(), Math::Vec3(0, 0, 0), 50.0f, false);
        source.Volume = GetEffectiveVolume(Category, 1.0f);
        m_AudioManager->UpdateAudioSource(source);
