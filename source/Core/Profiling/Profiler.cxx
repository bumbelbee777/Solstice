...
thread_local std::vector<std::pair<std::string, std::chrono::high_resolution_clock::time_point>> g_threadScopeStack;
...
void Profiler::BeginFrame() {
    ...
    // If the previous frame was left open, reset safely without recursive locking.
    if (m_rootScope) {
    ...
    m_frameThreadId = std::this_thread::get_id();
    m_rootScope.reset();
    m_currentScope = nullptr;
...
void Profiler::BeginScope(const char* name) {
    ...
    // Worker threads use per-thread accumulation to avoid corrupting the frame tree.
    if (std::this_thread::get_id() != m_frameThreadId) {
        BeginThreadScope(name);
        return;
    }
...
void Profiler::RenderOverlay(bool* pOpen) {
    ...
    FrameStats avgStats;
    {
        int count = std::min(60, static_cast<int>(m_frameHistory.size()));
        int startIdx = static_cast<int>(m_frameHistory.size()) - count;
        float totalFrameTime = 0.0f;
        float totalFPS = 0.0f;
        float totalGPUTime = 0.0f;
        ...
        for (int i = startIdx; i < static_cast<int>(m_frameHistory.size()); ++i) {
            ...
            totalGPUTime += frame.GPUTime;
            totalMemory += frame.MemoryUsage;
            maxPeakMemory = std::max(maxPeakMemory, frame.PeakMemory);
            for (const auto& pair : frame.ScopeTimes) totalScopeTimes[pair.first] += pair.second;
            for (const auto& pair : frame.GPUScopeTimes) totalGPUScopeTimes[pair.first] += pair.second;
            for (const auto& pair : frame.Counters) totalCounters[pair.first] += pair.second;
        }
        if (count > 0) {
            float invCount = 1.0f / static_cast<float>(count);
            avgStats.FrameTime = totalFrameTime * invCount;
            avgStats.FPS = totalFPS * invCount;
            avgStats.GPUTime = totalGPUTime * invCount;
            avgStats.MemoryUsage = totalMemory / count;
            avgStats.PeakMemory = maxPeakMemory;
            for (const auto& pair : totalScopeTimes) avgStats.ScopeTimes[pair.first] = pair.second * invCount;
            for (const auto& pair : totalGPUScopeTimes) avgStats.GPUScopeTimes[pair.first] = pair.second * invCount;
            for (const auto& pair : totalCounters) avgStats.Counters[pair.first] = pair.second / count;
        }
    }
...
void Profiler::BeginThreadScope(const char* name) {
    if (!m_enabled) return;
    g_threadScopeStack.push_back({name ? name : "ThreadScope", std::chrono::high_resolution_clock::now()});
}

void Profiler::EndThreadScope(const char* name) {
    if (!m_enabled || g_threadScopeStack.empty()) return;
    (void)name;

    const auto [scopeName, startTime] = g_threadScopeStack.back();
    ...
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        m_threadScopeTimes[threadId][scopeName] += duration;
    }
}
