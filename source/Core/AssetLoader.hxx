...
struct AssetLoadResult {
    std::vector<std::unique_ptr<::Solstice::Render::Mesh>> Meshes;
...
public:
    ...
    // Load an OBJ file and return all contained data
    // Path can be relative to asset path or absolute
    static AssetLoadResult LoadOBJ(const std::filesystem::path& Path);
    ...
    // Load just the first mesh from a file (supports glTF and OBJ)
    // Useful for simple cases where you only want one mesh
    static std::unique_ptr<::Solstice::Render::Mesh> LoadMesh(const std::filesystem::path& Path);
...
private:
    ...
    static std::tuple<std::unique_ptr<::Solstice::Render::Mesh>, ExtendedVertexData, std::unique_ptr<::Solstice::Arzachel::SkinWeights>> ConvertMesh(
        const void* GLTFModel,  // tinygltf::Model*
        int MeshIndex,
        const std::vector<int>* JointMapping = nullptr // Maps glTF joint indices to BoneID values
    );
    ...
    static std::unique_ptr<::Solstice::Arzachel::AnimationClip> ConvertAnimation(
        const void* GLTFModel, // tinygltf::Model*
        int AnimationIndex
    );
    ...
    static bool ExtractPrimitiveData(
        const void* GLTFModel,      // tinygltf::Model*
        ...
        const std::vector<int>* JointMapping, // Maps glTF joint indices to BoneID values
        uint32_t& IndexOffset
    );

    static Material ConvertObjMaterial(
        const ObjMaterial& ObjMat,
